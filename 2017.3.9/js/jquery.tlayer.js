;(function ($, window, undefined) {
    ...
    var methods = {
        ...
        init: function (options) {
            ...
            var settings = $.extend({
                ...
                bindDrag        : true,         //是否绑定弹窗拖动功能，默认绑定（需要引入jqueryUI或easyUI才能生效）
                bindResize      : true,         //是否允许调整弹窗高度（仅限有标题行的弹窗），默认允许
    ...
    var layerUtil = {
        ...
        excuteLayer: function (settings) {
            ...
            if ( $layer.draggable && settings.bindDrag ) {
                console.log("tlayer-draggable");

                layerUtil.bindDrag( $layer );
            }

            if ( header && settings.bindResize ) {
                layerUtil.bindResize( $layer );
            }
        ...
        header: function (layerID) {
            ...
            var height = ( parseInt(h.height) || 30 ) - 6 + "px";

            //生成头部区域HTML
            var headerHTML  =   '<div class="layer-box-header" style="height:'+height+';line-height:'+height+';">'+
                                    '<h4 style="white-space: nowrap;text-overflow: ellipsis;overflow: hidden;width:'+(parseFloat(settings.width) - 60)+'px;">'+(h.text || "")+'</h4>'+
        ...
        buttons: function (layerID, buttons, block) {
            ...
            for (var i = 0; i < bLen; i++) {
                ...
                var attrs = {
                    ...
                    "style"         : ' style="'+(button.width ? 'width:'+button.width+';' : '')+(button.buttonBGImg ? 'background-image:url('+button.buttonBGImg+')' : '')+'"',
        ...
        /**
         * 为弹窗绑定拖动事件( 需要引入easyUI或JqueryUI )
         * @param  {object} $layer layer弹出框的jQuery对象
         * @return 无
         */
        bindDrag: function ( $layer ) {
            var $table = $layer.find(".layer-box-table"),
                $body = _top.$("body"),
                $headerWrapper = $table.find(".layer-box-header"),
                $header = $headerWrapper.find("h4"),
                $inner = $table.find( ".layer-box-content-inner" ),
                $block = $('<div class="layer-box-block"></div>');

            var mouse = { flag: false };

            // 添加遮挡层，拖动时显示，停止后隐藏，防止光标移入弹窗的iframe里时拖动事件不触发的问题
            $table.append( $block );
                ...
                mouseenter: function(e) {
                    if ( $table.css( "position" ) == "relative" ) { 
                        $table.css( {
                            "position": "absolute",
                            "margin": 0,
                            "left": ( 1 - $table.width() / $body.width() ) * 100 / 2 + "%",
                        } );
            ...
            function fixDrag() {
                ...
                if ( $table.height() == cHeight ) {
                    top += $table.find(".layer-box-inner > *:eq(0)").position().top;
                ...
                var tHeight = $table.height(),
                    maxLeft = cWidth > tWidth ? ( cWidth - tWidth ) : 0,
                    maxTop = cHeight > tHeight ? ( cHeight - tHeight ) : 0;
         ...
         * @return 无
         */
        bindResize: function ( $layer ) {
            var $tab = $layer.find( ".layer-box-table" ),
                $el = $tab.find( ".layer-box-content-inner" ),
                $handle = $tab.find(".layer-box-header"),
                $block = $tab.find( '.layer-box-block' ),
                $body = $tab.closest( '.layer-box-wraper' ),
                tabs = ($tab[0] || {}).style,
                els = ($el[0] || {}).style,
                inners = ($tab.find( ".layer-box-inner" )[0] || {}).style;
            ...
            function mouseMove(e) {
                ...
                // timer = setTimeout( function () {
                    inners.height = parseFloat( inners.height ) - offsetY * 2 + 'px';
                    els.height = elh - offsetY * 2 + 'px';
                    tabs.top = parseFloat( tabs.top ) + offsetY * 2 + "px";
                    offsetY = 0;
                // }, 50 );
            }
        ...
        hideLayer: function (layerID, fn) {
            ...
            //出栈
            layerData.stack.splice(layerData.stack.indexOf(layerID), 1);
