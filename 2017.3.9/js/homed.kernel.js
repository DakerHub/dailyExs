;(function ($, window, undefined) {
        ...
        info: function ( options ) {
            ...
            handler.__public__.getData( options, function ( data ) {
                ...
                if ( data.ret == 0 ) {
                    ...
                    // 该用户无系统权限，强制返回首页/登录页
                    if ( i == 0 ) {
                        _top.location.href = CONFIG.homepage || gdc.homePageAddr;
                        return ;
                    }
                            ...
                            if (toolData) {
                                ...
                                if ($this.hasClass("tool-select-option")) {
                                    ...
                                    var $tool = $this.closest(".appedit-tools-wraper"),
                                        ...
                                        multiple = $tool.data("toolData").multiple || false;
                    ...
                    case "select":
                        ...
                        $this.removeClass(disSelect)
                            .find(".tool-quicker-wraper .tool-icon").removeClass(function () {
                                ...
                                for ( var i in quickClass ) {
                                    if ( quickClass.hasOwnProperty( i ) ) {
                                        cls.push( quickClass[i] );
                                    }
                                }
        ...
        template: function (tool) {
            ...
            switch (tool.type) {
                ...
                case "select":
                        ...
                        var html =  '<div'+(tool.toolID?' id="'+tool.toolID+'"':'')+' data-tool-type="select" class="appedit-tools-wraper appedit-tool-'+orientation+' appedit-tool-'+tool.type+(tool.disabled?disSelect:"")+(tool.multiple?" appedit-select-multiple":"")+'">';
                        ...
                        var options =   '<div class="tool-options">';
                                        ...
                                        for (var i = 0, len = ops.length; i < len; i++) {
                                            ...
                                            var option =    '<div id="'+op.toolID+'" data-tool-type="button" value="'+(op.value||"")+'" class="tool-select-option'+(op.disabled?disTool:"")+(tool.multiple&&op.selected?" active":"")+'">'+
    ...
    handler.__public__ = {
        ...
        sensitive: function (data) {
            ...
            var attrs = {
                seriesname: '"中文名称"',
                videoname: '"中文名称"',
                seriesenglishname: '"英文编码"',
                videoenglishname: '"英文编码"',
                secondlangname: '"并列名称"',
                publishcompany: '"发行商"',
                screenwriter: '"编剧"',
                director: '"导演"',
            ...
            if (result !== true) {
                var retarr = retmsg.split( "!" );

                for( var i = 0; i < retarr.length - 1; ++i ){
                    var msg = retarr[i].split( ":" ),
                        word = $.trim( msg[0] );
                    ...
                    if( msg.length == 2 ){
                        ...
                        retmsgs.push( attr + '包含敏感词' + values.join("、") );
                    }
                }
                ...
                if (util.isFunction(_top.$.alert)) {
                    _top.$.alert({
                        content: {
                            html:   '<p>'+tip+'<br />\
                                        <span style="color:#646464;font-size:12px;">' + ( retmsgs.join( "<br />" ) || retmsg ) + '</span></p>'
                        }
                    });
                } else {
                    alert( tip + '\r\n' + retmsg );
        ...
        getData: function (options, fn) {
            ...
            // 是否去掉 key 中的下划线，默认不去掉
            options.rmunderline = options.rmunderline !== undefined ? options.rmunderline : false;
            ...
            if ( !options.noToken && options.data.accesstoken === undefined ) {
                //设置请求数据中的accesstoken
                options.data.accesstoken = methods.data( "token" );
            } else if ( options.data.accesstoken === false ) {
                //删除请求数据中的accesstoken
                options.noToken = true;
                delete options.data.accesstoken;
            }
            ...
            var ajaxSettings = $.extend({}, options, {
                success: function (data) {
                    ...
                    // 敏感词检测
                    if (!_this.sensitive(data)) { 
                        
                        util.runFunction( options.sensitive || function () {
                            _top.$( ".layer-box-block-buttons .saving" ).removeClass("saving").removeData("saving"); 
                        } );
        ...
        setData: function (options, fn) {
            ...
            var ajaxSettings = $.extend({}, options, {
                ...
                //默认超时时间
                timeout: options.timeout || 20000,
                data: options.type == "POST" && options.stringify ? JSON.stringify(options.data)/*.replace(/%/g, "%25")*/ : options.data,
                url: (Api[options.url] || options.url),
                error: util.isFunction(options.error) ? options.error : function () {}
            });
    ...
    var tPager = {
        ...
        init: function (option) {
            ...
            var indexColumn = option.indexColumn || "pageidx",
                sizeColumn = option.sizeColumn || "pagenum",
                pageType = option.pageType || 1;

            option.indexColumn = indexColumn;
            option.sizeColumn = sizeColumn;
            option.pageType = option.pageType;
            ...
            if ( option.select !== false ) {
                ...
                if ( OPERATOR == "fujian" && !pagenumCookie[url] ) {
                    pagenumCookie[url] = 100;
                }

                    //一页数据量
                var pagenum = +(pagenumCookie[url] || pageInfo[ sizeColumn ] ),
                ...
                pageInfo[sizeColumn] = pagenum;

            }
            
            $wraper.data("option", option);

            var page = pageType == 2 ?  parseInt(option.request.data[indexColumn] / option.request.data[sizeColumn]) + 1 : option.request.data[indexColumn];

            //生成分页导航HTML代码，并默认获取 pageidx 页的数据
            tPager.getPage.call( this, page, option );
        ...
        pageNum: function () {
            ...
            var option = $( this ).data("option") || {},
                indexColumn = option.indexColumn;

            return !$.isEmptyObject( option ) ? option.request.data[indexColumn] : false;

        },
        pageSize: function () {

            var option = $( this ).data("option") || {},
                sizeColumn = option.sizeColumn;

            return !$.isEmptyObject( option ) ? option.request.data[sizeColumn] : false;
        ...
        getPage: function (page, option) {
            ...
            var request = option.request.data,
                ...
                pageidx = request[indexColumn],
                pagenum = request[sizeColumn];

            if ( pageType == 2 ) {
                maxPage = ( maxPage - 1 ) * pagenum;
            }

            switch (page) {
                case "-1":
                case "+1":
                    pageidx = pageidx + (+page) * ( pageType == 2 ? pagenum : 1 );
                    break;
                default :
                    pageidx = pageType == 2 ? ( page - 1 ) * pagenum : page;
                    break;
            }

            if (pageidx >= 0 && ( !maxPage || pageidx <= maxPage)) {
                request[indexColumn] = +pageidx;
                util.runFunction( option.onPaging, $this );
                handler.__public__.getData(option.request, function ( data ) {
                    var totalColumn = (option.totalColumn || "total").split( "." ),
                        curridx = pageType == 2 ? parseInt( pageidx / pagenum ) + 1 : pageidx,
                        total;

                    for ( var i = 0, len = totalColumn.length; i < len; ++i ) {
                        var key = totalColumn[ i ];

                        total = i == 0 ? data[ key ] : total[ key ];

                        if ( typeof total == "undefined" ) break;
                    }

                    option.total = total || 0;

                    // 判断当页数是否超过最大页数
                    if( curridx > 1 && pagenum * (curridx - 1) >= option.total ){
        ...
        initPageNavHTML: function (option) {
            ...
            var pageInfo = option.request.data,
                ...
                //一页数据量
            var pagenum = pageInfo[sizeColumn],
                //当前页码
                pageidx = pageInfo[indexColumn],
                //总数据量
                total = option.total,
                // 分页下拉框
            ...
            if ( pageType == 2 ) {
                pageidx = parseInt( pageidx / pagenum ) + 1;
            }
    ...
    var util = {
         ...
         */
        isURL: function ( str ){
            return!!str.match(/(((^ht?t?p?d?f?s?:(?:\/\/)?)(?:[-;:&=\+\$,\w]+@)?.+|(?:www.|[-;:&=\+\$,\w]+@).+)((?:\/[\+~%\/.\w\u4e00-\u9fa5-_]*)?\??(?:[-\+=&;%@.\w\u4e00-\u9fa5_]*)#?(?:[\w\u4e00-\u9fa5]*))?)$/g);
        },
    };
